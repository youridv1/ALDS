...
def multiStartLocalSearch4CoG(coordinationGraph, noIterations):
    ...
    for _ in range(noIterations):
        randomSolution = [random.randrange(3)] * len(coordinationGraph.nodesAndConnections.keys()) # Random state * amount of nodes
        randomSolution = localSearch4CoG(coordinationGraph, randomSolution) # Execute solution
        newReward = coordinationGraph.evaluateSolution(randomSolution) # Get new reward

        if newReward > reward: # If reward is better
            newSolution = randomSolution # Improve current solution
            reward = newReward

    # Return
    return newSolution, reward
...
def iteratedLocalSearch4CoG(coordinationGraph, pChange, noIterations):
    ...
    for _ in range(noIterations): # For every iteration
        newSolution = solution # Current solution

        for i in coordinationGraph.nodesAndConnections.keys(): # For every node
            r = random.uniform(0, 1) # Random probability
            if r < pChange: # If
                newSolution[i] = random.randrange(3)

        newSolution = localSearch4CoG(coordinationGraph, newSolution) # Execute new solution
        newReward = coordinationGraph.evaluateSolution(newSolution) # Get new reward
...
print("LS:  " + str(cog.evaluateSolution(localSearch4CoG(cog, [2]*nVars))))
print("MLS: " + str(multiStartLocalSearch4CoG(cog, 10)[1]))
print("ILS: " + str(iteratedLocalSearch4CoG(cog, 0.0, 10)[1]))
...
reward = 0
for _ in range(1000):
    currentReward = iteratedLocalSearch4CoG(cog, p, 10)[1]
    if currentReward > reward: reward = currentReward; bestP = p
    p += 0.001
